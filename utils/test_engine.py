...
def test_engine(__C, dataset, state_dict=None, validation=False):
    ...
    if __C.WIDTH and __C.DEPTH:
        candidate_submodels = [[__C.WIDTH, __C.DEPTH]]
    else:
    ...
    for width, depth in candidate_submodels:
        for step, (
                frcn_feat_iter,
                grid_feat_iter,
                bbox_feat_iter,
        ...
        if validation:
            ...
            if __C.CKPT_PATH is not None:
                result_eval_file = __C.RESULT_PATH + '/result_run_' + __C.CKPT_VERSION + '_hidden_size_' + str(
                    Fraction(width) * __C.HIDDEN_SIZE) + '_layer_num_' + str(Fraction(depth) * __C.LAYER)
            else:
                result_eval_file = __C.RESULT_PATH + '/result_run_' + __C.CKPT_VERSION + '_hidden_size_' + str(
                    Fraction(width) * __C.HIDDEN_SIZE) + '_layer_num_' + str(
                    Fraction(depth) * __C.LAYER) + '_epoch' + str(__C.CKPT_EPOCH)
        ...
        logfile.write(
            'Submodel: width = ' + str(width) + ',        depth = ' + str(depth) + '\n'
        )
        logfile.close()
        print('Submodel: width = ' + str(width) + ',        depth = ' + str(depth))
